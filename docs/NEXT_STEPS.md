...
The Phase 4 workspace client and the image/theme slice are implemented. The client source now has a 40-theme catalog, the selected theme-colored rounded-eye mark, image paste/drop, and a full-image inspector. Graphics retention and real 4K detach/reattach have been exercised on Windows/WSL and Linux; Windows uses a pinned Microsoft ConPTY runtime because the stock runtime discarded graphics. Remote SSH transport is the next implementation phase. Native Windows qualification of the expanded catalog, broader Mac UI/input qualification, resource measurement, and remaining physical-input/display gates stay open; earlier hosted results below remain dated evidence, not claims about unrun current CI.
