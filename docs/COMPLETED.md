...
- Every GUI connection path and `compi-probe` workspace/session/tab/pane/surface/soak/shutdown command uses the same local-or-SSH target. The headless probe remains the authoritative discovery surface for hierarchy, process lifetime, status, attachment, dimensions, and errors. Agent-specific metadata is intentionally limited to a future explicit, non-secret launch descriptor for a concrete consumer.
- Remote image paste/drop uploads to the remote daemon in 256 KiB chunks with a 64 MiB per-image limit, four active uploads per connection, exact offsets, SHA-256 verification, disconnect cleanup, and a private content-addressed store capped at 512 MiB and 4096 files. The UI inserts only the safely quoted remote path and never executes it automatically.
- Main-screen Kitty placements now follow logical text across width-changing reflow and expire with retained history. Required Kitty direct RGB/RGBA/PNG, chunking, zlib, placement/query/delete, clipping, resize, history, and reconnect behavior is explicit. Unknown Kitty controls return `ENOTSUP`; sixel and the documented advanced Kitty features remain outside the baseline.
- The current Windows source built an unsigned setup and portable ZIP after the isolated installer lockfile was refreshed. WiX validation completed with the known per-user ICE91 warnings. Artifact hashes matched, the ZIP retained both Compi executables, the pinned Microsoft-signed ConPTY pair and both licenses, and two native launches preserved daemon/surface/process identities.
- GitHub's completed job record for [release run 34711439577](https://github.com/cloudboy-jh/compi/actions/runs/34711439577) confirms that Windows build, portable and installed launch, repair, uninstall, and artifact upload all succeeded before the run was cancelled during `rust-cache` post-processing. The Mac build, copied-bundle launch/reconnect smoke, and artifact upload also succeeded. Both artifacts remain retained by Actions.
...
- [Release run 34711439577](https://github.com/cloudboy-jh/compi/actions/runs/34711439577), source commit `21a3af9`, passed the ARM64 Mac build and copied-DMG-bundle LaunchServices/reconnect smoke. Its job record also confirms the Windows installer and portable build, installed/portable launch, repair, uninstall, and artifact upload all succeeded; the overall run was cancelled only during cache post-processing.
