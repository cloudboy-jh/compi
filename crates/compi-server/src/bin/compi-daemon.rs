...
#[cfg(any(windows, unix))]
fn run() -> compi_server::Result<()> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    match args.as_slice() {
        [] => compi_server::daemon::run(None),
        [flag, instance] if flag == "--instance" && !instance.is_empty() => {
            compi_server::daemon::run(Some(instance))
        }
        [flag] if flag == "--check-system" => compi_server::launch::check_system(),
        [flag] if flag == "--shutdown" => shutdown_daemon(),
        #[cfg(windows)]
        [flag] if flag == "--supervise" => {
            compi_server::supervisor::supervise(&std::env::current_exe()?)
        }
        #[cfg(windows)]
        [flag] if flag == "--install-task" => {
            compi_server::supervisor::install(&std::env::current_exe()?)?;
            println!("registered {}", compi_server::supervisor::TASK_NAME);
            Ok(())
        }
        #[cfg(windows)]
        [flag, path, user_sid] if flag == "--write-task-xml" => {
            compi_server::supervisor::write_task_xml(
                &std::env::current_exe()?,
        ...
        [flag, path] if flag == "--remove-task-xml" => {
            compi_server::supervisor::remove_task_xml(std::path::Path::new(path))
        }
        #[cfg(windows)]
        [flag] if flag == "--uninstall-task" => {
            compi_server::supervisor::uninstall()?;
            println!("removed {}", compi_server::supervisor::TASK_NAME);
            Ok(())
        }
        #[cfg(windows)]
        [flag] if flag == "--activate-task" => compi_server::supervisor::activate(),
        #[cfg(unix)]
...
#[cfg(any(windows, unix))]
fn shutdown_daemon() -> compi_server::Result<()> {
    use compi_server::client::DaemonClient;
    use std::thread;
