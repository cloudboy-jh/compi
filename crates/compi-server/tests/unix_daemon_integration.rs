...
impl DaemonGuard {
    ...
    fn wait_ready(&mut self) {
        ...
        let names = compi_server::identity::instance_names(Some(&self.instance)).unwrap();
        loop {
            if compi_server::pipe::connect(&names.pipe, Duration::from_millis(100)).is_ok() {
                return;
            }
            assert!(
    ...
    fn unhandshaken_client(&self) -> DaemonClient {
        let names = compi_server::identity::instance_names(Some(&self.instance)).unwrap();
        let connection = compi_server::pipe::connect(&names.pipe, Duration::from_secs(2)).unwrap();
...
impl Drop for DaemonGuard {
    fn drop(&mut self) {
        if self.child.try_wait().ok().flatten().is_none() {
            if let Ok(names) = compi_server::identity::instance_names(Some(&self.instance))
                && let Ok(connection) =
                    compi_server::pipe::connect(&names.pipe, Duration::from_millis(100))
            {
...
fn local_endpoint_is_private_and_duplicate_daemon_cannot_take_it_over() {
    let mut daemon = DaemonGuard::start();
    let names = compi_server::identity::instance_names(Some(&daemon.instance)).unwrap();
